...
// Test script details: Retrieving details for all licenses
...
const apiUrl = testData.getalllicenseurl;
const authToken = 'your-authentication-token-here';

test('Test get all licenses with token authentication', async ({ request }) => {
  ...
  console.log('Response Body:', responseBody);
  expect(firstProduct).toHaveProperty('type', 'licenses');
  expect(firstProduct).toHaveProperty('attributes');
  expect(firstProduct.attributes).toHaveProperty('status', 'ACTIVE');
