...
// Test script details: Retrieving details for all machines
...
const apiUrl = testData.getallmachinesurl;
const updateplatforms = testData.updateplatform;
const authToken = 'your-authentication-token-here';

test('Test get all machines with token authentication', async ({ request }) => {
  ...
  expect(firstProduct).toHaveProperty('type', 'machines');
  expect(firstProduct).toHaveProperty('attributes');
  expect(firstProduct.attributes).toHaveProperty('platform', updateplatforms);
